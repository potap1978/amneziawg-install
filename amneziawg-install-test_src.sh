...
function installQuestions() {
	...
	until [[ ${KEEPALIVE} =~ ^[0-9]+$ ]] && [ "${KEEPALIVE}" -ge 0 ] && [ "${KEEPALIVE}" -le 65535 ]; do
		read -rp "$(echo -e "${GREEN}Keepalive interval${NC}") [0-65535]: " -e -i 0 KEEPALIVE
	done
...
function uninstallAmneziaWG() {
	...
	if [[ $REMOVE == 'y' ]]; then
		...
		if [[ ${OS} == 'ubuntu' ]]; then
			# Удаление вручную собранных компонентов
			rm -rf ~/awg-src
			dkms remove -m amneziawg -v 1.0.0 --all
			rm -f /usr/local/bin/awg
			rm -f /usr/local/bin/awg-quick
			apt remove -y git build-essential ncurses-dev xz-utils libssl-dev bc flex libelf-dev bison make mokutil dkms qrencode iptables
		elif [[ ${OS} == 'debian' ]]; then
