...
# Установка зависимостей и патча ядра ТОЛЬКО для Ubuntu 25.04
	if [[ ${OS} == 'ubuntu' ]]; then
	    if grep -q "plucky" /etc/os-release; then
        	setupKernelAndDependencies
	    else
        	echo "Skipping kernel setup for non-Ubuntu 25.04 systems"
	    fi
	fi
